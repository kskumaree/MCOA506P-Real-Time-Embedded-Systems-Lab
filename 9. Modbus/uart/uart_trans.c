...
void GPIO_init(void);
void USART_write(volatile uint8_t *buffer,volatile uint32_t nBytes);
...
USART2->DR  = buffer[i]&0x1FF;   //WRITING TO DR FLAG CLEARS TXE FLAG
}
...
GPIO_init();
USART2_init();
volatile uint8_t *buffer_w;
volatile uint8_t data_w = 0xA0;
