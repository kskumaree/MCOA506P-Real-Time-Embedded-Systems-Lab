...
void ADC_Config(void)
{
	...
	ADC->CCR |= (0UL<<16);  //Pre-scale clock to ADC by 4
	...
	ADC1->SMPR2 |= (7UL<<0);//Select sampling time as 480 cycles for channel 1
	GPIOA->MODER |= (3UL<<0); //Configure PA0 in analog mode
	ADC1->SQR3 |= (0<<0);//Select channel 0 in ADC (Corresponds to PA1)
...
void dma_config(void)
{
	...
	DMA2_Stream0->PAR=reg;//Select peripheral address as ADC1
